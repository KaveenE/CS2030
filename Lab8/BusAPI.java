 ...
 * cs2030-bus-api.herokuapp.com to retrieve: - bus services that serve a given
 * bus stop - bus stop visited by a bus service.
 *
...
class BusAPI {
	...
	/**
	 * Given a URL, synchrnously obtained the HTTP response string from the URL. It
	 * returns an emptry string and prints an error message if the URL is invalid
	 * (not the best behavior).
	 * 
	 * @param url The URL to query
	 * @return The HTTP resposne body, or an empty string if the query fails.
	 */
	 ...
	 * @return The string returned by the CS2030 BUS API service listing the bus
	 *         stops that are serviced at this bus. Return an empty string if
	 *         something go wrong.
	 */
	 ...
	 * @return The string returned by the CS2030 BUS API service listing the bus
	 *         services that stopped at this bus stop; an empty string if the API
	 *         query failed.
	 */
