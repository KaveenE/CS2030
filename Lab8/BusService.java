...
class BusService {
	...
	public CompletableFuture<Set<BusStop>> getBusStops() {
		...
		CompletableFuture<Set<BusStop>> stopsF = BusAPI.getBusStopsServedBy(serviceId).thenApply(str -> {
	...
	public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
		return getBusStops().thenApply(set -> {
			return set.stream().filter(stop -> stop.matchName(name)).collect(Collectors.toSet());
		});
