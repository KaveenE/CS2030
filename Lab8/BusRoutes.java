...
class BusRoutes {
	...
	public CompletableFuture<String> description() {
		...
		for (BusService service : services.keySet()) {
			...
			result.thenCombine(describeService(service, stops), (str1, str2) -> str1 + str2);
		}
		return result;
	}
	...
	public CompletableFuture<String> describeService(BusService service, CompletableFuture<Set<BusStop>> stops) {
		return stops.thenApply(stops1 -> {
			if (stops1.isEmpty()) {
				return "";
			} else {
				return stops1.stream().filter(stop -> stop != this.stop).reduce("- Can take " + service + " to:\n",
						(str, stop) -> str += "  - " + stop + "\n", (str1, str2) -> str1 + str2);
			}
		});
